...
from src.config import inicializar_llm_claude
...
class EstadoProyecto(TypedDict):
    ...
    respuesta: str      # La respuesta final generada por Claude
...
def nodo_generar_recomendacion(state: EstadoProyecto):
    # Invocamos la configuración de Claude y el prompt del Integrante A
    llm = inicializar_llm_claude()
...
def construir_grafo():
    ...
    workflow.add_node("analista_claude", nodo_generar_recomendacion)
    
    # Definimos el flujo lineal inicial: Inicio -> Nodo -> Fin
    workflow.add_edge(START, "analista_claude")
    workflow.add_edge("analista_claude", END)
